...
#       the count. If the script receives a QUIT signal, tell the user they forfeit the game
...
function trap_quit() {
    echo "You forfeit the game. Exiting immediately."
    exit 1
}

# Add traps for INT and QUIT signals
trap trap_ctrlc INT
trap trap_quit QUIT
